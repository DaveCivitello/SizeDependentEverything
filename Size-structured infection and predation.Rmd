...
* Size-selective predation (8/22/2022)
  + Implemented as an exponential continous effect, when negative coefficient, based on snail length, when positive, based on difference between maximum length and current shell size
  ...
  for(tick in 1:pars["timespan"]){
    ...
    if(dim(state$Snails[[tick]])[1] > 0){
      ...
      # pred_as = pars["pred_a0"]*exp(pars["pred_aL"]*(snail.stats[,"L"]))*(1 + (snail.stats[,"Cercs"]>0)*pars["pred_alpha_Inf"])
            ...
            # Determine snail mortality rates due to size- and infection-selective predation
      pred_as = ifelse(pars["pred_a0"] < 0,
        pars["pred_a0"]*exp(pars["pred_aL"]*(snail.stats[,"LG"]))*(1 + (snail.stats[,"Cercs"]>0)*pars["pred_alpha_Inf"]),
        pars["pred_a0"]*exp(-pars["pred_aL"]*(pars["LM"] -  snail.stats[,"LG"]))*(1 +   
        (snail.stats[,"Cercs"]>0)*pars["pred_alpha_Inf"]))
...
facsim_result = DEBIBM_factorial_simulator(OoI=Outputs_of_Interest, PoI=Parameters_of_Interest, mins = c(0, -0.1), maxs = c(500, 0.1), levels = 11, iterations=4)
